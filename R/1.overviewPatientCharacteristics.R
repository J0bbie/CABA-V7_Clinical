...
data.Patient$clinicalData <- data.Patient$clinicalData %>%
    # Add AR-V7 status (on baseline).
    dplyr::left_join(data.Patient$Overview %>% dplyr::distinct(`Subject Number`, `Inclusion (Treated with Caba)`, `AR-V7 (Baseline)`, `CTC Count (Baseline – 7.5mL)`)) %>%
...
print(
    tableOne,
    test = F,
    nonnormal = c('Age at registration', 'Total Gleason', 'CTC Count (Baseline – 7.5mL)', 'M-stage primary diagnosis', 'postTreatment_Clean', 'PSA at primary diagnosis [ug/L]'),
    exact = c('n'),
    smd = F,
    minMax = T,
    missing = T,
...
dataHormonal <- data.Patient$Overview %>% dplyr::distinct(`Subject Number`, `Inclusion (Treated with Caba)`, `AR-V7 (Baseline)`) %>% 
    dplyr::mutate(`AR-V7 (Baseline)` = ifelse(`Inclusion (Treated with Caba)` == 'Yes' & `AR-V7 (Baseline)` == 'Pos.', 'Main - Pos.', `AR-V7 (Baseline)`)) %>% 
    dplyr::left_join(
        data.Patient$priorHormonal %>% 
            dplyr::filter(grepl('Abi|Enza', `Type of prior hormonal treatment`)) %>% 
            dplyr::distinct(`Subject Number`, `Type of prior hormonal treatment`) %>% 
            dplyr::group_by(`Subject Number`) %>% 
            dplyr::summarise(`Type of prior hormonal treatment` = paste(`Type of prior hormonal treatment`, collapse = ' & ')) %>% 
            dplyr::ungroup()
    )
...
tableOne <- tableone::CreateTableOne(
    vars = c(
        'Type of prior hormonal treatment'
    ),
    strata = c("AR-V7 (Baseline)"),
    data = dataHormonal,
    smd = F,
...
dataAUC %>% 
    ...
    ggplot2::stat_summary(fun = DescTools::Gmean, colour='black', geom='text', size = 3, show.legend = FALSE, vjust=-4, angle = 90, hjust = .5) +
    ...
    ggplot2::scale_y_continuous(limits = c(0, 500), breaks = seq(0, 500, 100), expand = c(0,0)) +
    theme_Job
